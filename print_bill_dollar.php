...
<div class="page-header" style="text-align: center">
    <div class="table-responsive">
        <table class="table table-borderless">
            ...
            <tr>
                <td colspan="3" class="total_Due">Total Due: $ <?php echo preg_replace("/(\d+?)(?=(\d\d)+(\d)(?!\d))(\.\d+)?/i", "$1,", round(($total_recv_amount - $paid_amount),2)) ?></td>
            </tr>
        </table>
...
<table class="table">
    ...
    <tr>
        ...
        <td colspan="2">
            ...
            ?>
            <p class="tax_Vat">Tax/Vat (<?php echo $vat ?>%)<span> &emsp;$ <?php echo preg_replace("/(\d+?)(?=(\d\d)+(\d)(?!\d))(\.\d+)?/i", "$1,", round($vat_am,2)) ?></span></p>
            <hr class="horizontal_Line_Color">
            <p class="discount_Color">Discount(<?php $overall_discount;
                ?>%)<span> &emsp; $ <?php echo preg_replace("/(\d+?)(?=(\d\d)+(\d)(?!\d))(\.\d+)?/i", "$1,", round($dis,2)) ?></span></p>
            <hr class="horizontal_Line_Color">

            <p class="tax_Vat">Paid<span> &emsp; $ <?php echo preg_replace("/(\d+?)(?=(\d\d)+(\d)(?!\d))(\.\d+)?/i", "$1,", round($paid_amount,2)) ?></span></p>
            <hr class="horizontal_Line_Color">
            <br>
            <br>
            <br>
        ...
        <td colspan="2">
            <p class="tax_Vat">SUB TOTAL<span> &emsp;$ <?php echo preg_replace("/(\d+?)(?=(\d\d)+(\d)(?!\d))(\.\d+)?/i", "$1,", round($total_amount,2)) ?></span></p>
            <hr class="horizontal_Line_Color">
            <p class="discount">GRAND TOTAL<span> &emsp; $ <?php echo preg_replace("/(\d+?)(?=(\d\d)+(\d)(?!\d))(\.\d+)?/i", "$1,", round($total_recv_amount,2)) ?></span></p>
            <hr class="horizontal_Line_Color">

            <p class="discount_Color">Due<span> &emsp; $ <?php echo preg_replace("/(\d+?)(?=(\d\d)+(\d)(?!\d))(\.\d+)?/i", "$1,", round(($total_recv_amount - $paid_amount),2)) ?></span></p>
            <hr class="horizontal_Line_Color">
            <br>
            <br>
            <br>
